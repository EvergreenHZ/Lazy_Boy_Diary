...
#pragma omp critical // modify the record by a single thread
	...
	void npddetect::initTrackers(Mat& frame)
	{
		vector< Ptr<Tracker> > algos;
		for (int i = 0; i < m_Xs.size(); i++) {
			ROIs.push_back(Rect(m_Xs[i], m_Ys[i], m_Ss[i], m_Ss[i]));

			algos.push_back(createTrackerByName("KCF"));
			//algos.push_back(Ptr<Tracker>(TrackerKCF::create()));
		}
		trackers.add(algos, frame, ROIs);
		ROIs.clear();
		return ;  // you should clear the rois
	}
}
