...
namespace npd{
	...
	npddetect::npddetect() :  frame(480, 640, CV_8UC1, Scalar::all(0))
	{
	...
	npddetect::npddetect(int minFace, int maxFace) : frame(480, 640, CV_8UC1, Scalar::all(0))
	{
	...
	int npddetect::scan(const unsigned char* I, int width, int height)
	{
		...
		//frame(height, width, CV_8UC1, (void*)I);
		
		Mat tmp(height, width, CV_8UC1, (void*)I);
		frame = tmp;
		pMOG2->apply(frame, fgMask);
...
#pragma omp parallel for
			...
			for(int r = 0; r < rowMax; r += winStep) // slide in row
			{
				...
				for(int c = 0; c < colMax; c += winStep, pPixel += winStep) // slide in column
				{
					const unsigned char *t_center =maskI + r * width + c * winStep + m_model.m_winSize[k] / 2 + m_model.m_winSize[k]/ 2 * width;
					if ( *(t_center) + *(t_center - 1) + *(t_center +1)  < 128) continue;
