...
namespace npd{
	...
	npddetect::npddetect(): frame(480, 640, CV_8UC1)
	{
		init(m_model.m_objSize, 5000);
		pMOG2 = createBackgroundSubtractorMOG2(); //MOG2 approach
	}

	npddetect::npddetect(int minFace, int maxFace): frame(480, 640, CV_8UC1)
	{
	...
	int npddetect::detect(const unsigned char* I, int width, int height)
	{
		...
		printf("Time to scan the picture: %lf\n", t);

		// Merge rect.
		t = (double)cvGetTickCount();
		m_numDetect = filter();
		t = ((double)cvGetTickCount() - t) / ((double)cvGetTickFrequency()*1000.) ;
		printf("Time to merge rects: %lf\n", t);
	...
	int npddetect::scan(const unsigned char* I, int width, int height)
	{
		...
		//frame(height, width, CV_8UC1, I);
		//frame.data = const_cast<unsigned char*> (I);
		//const unsigned char *xxx = I;
		frame = Mat(640, 480, CV_8UC1, xxx);
		namedWindow("Frame");
		imshow("Frame", frame);
		pMOG2->apply(frame, fgMask);
		const unsigned char* maskI = fgMask.data;
		int k;
		//double tcnt = 0.0f;
		for(k = 0; k < m_model.m_numScales; k++) // process each scale
		{
			...
			// determine the step of the sliding subwindow
			int winStep = (int) floor(m_model.m_winSize[k] * 0.2);
			if(m_model.m_winSize[k] > 40) winStep = (int) floor(m_model.m_winSize[k] * 0.05);
			...
			const int proportion = 255 * m_model.m_winSize[k] / 4;
#pragma omp parallel for
			...
			for(int r = 0; r < rowMax; r += winStep) // slide in row
			{
				...
				for(int c = 0; c < colMax; c += winStep, pPixel += winStep) // slide in column
				{
					const unsigned char *tpPixel = maskI + (c + r) * width;
					const Mat tmp_mat(m_model.m_winSize[k], m_model.m_winSize[k], CV_8UC1, *tpPixel);
					if ( trace(tmp_mat)[0] < proportion ) continue;
					// check current window, trace
					int treeIndex = 0;
					float _score = 0;
					int s;
...
#pragma omp critical // modify the record by a single thread
	...
	int npddetect::filter()
	{
		...
		for(i = 0; i < m_numScan; i++)  // for all candicate rects
		{
	...
	{
		if(parent[i] != i)  // find fixed point
	...
	int npddetect::partition(char* predicate, int* root)
	{
		...
		int root_i, root_j;
		for(i = 0; i < m_numScan; i++)
			m_Tparent[i] = i;  // single tree, one node, root, itself
		m_Trank[i] = 0;
		...
		for(i = 0; i < m_numScan; i++)
		{
			...
			{
				if(predicate[ni] == 0)  // no intersection
					continue;  // predicate is a symmetric matrix

				root_i = findRoot(m_Tparent, i);
				root_j = findRoot(m_Tparent, j);

				if(root_i != root_j)  // intersection but not in the same tree, merge
				{
...
/*void npddetect::detectImage(const char *imgLists)
  {
  ifstream infile(imgLists);
  string line;
  vector<string> imgName;
  while (getline(infile, line)) {  // read all the images

  imgName.push_back(imgName);
  }

//vector<Rect> rects;
ofstream myfile("mark.txt");
for (int i = 0; i < imgName.size(); i++) {
Mat img = imread(imgName[i].c_str());  //read the image, actually, it is grayscale
Mat gray;
gray = img;
int n = detect(gray.data, gray.cols, gray.rows);

for (int j = 0; j < n; j++) {  // write the result
string mark;
mark += to_string(m_Xs[j]);
mark += " ";
mark += to_string(m_Ys[j]);
mark += " ";
mark += to_string(m_Ss[j]);
mark += " ";
mark += to_string(m_Ss[j]);
myfile<<mark<<endl;
}
