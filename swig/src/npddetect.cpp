...
namespace npd{
	...
	vector< int >& npddetect::getXs() { return m_Xs;}
	vector< int >& npddetect::getYs() { return m_Ys;}
	vector< int >& npddetect::getSs() { return m_Ss;}
	vector< float >& npddetect::getScores() { return m_Scores;}

	int* npddetect::getX() {return m_Xs.data();}
	int* npddetect::getY() {return m_Ys.data();}
	int* npddetect::getS() {return m_Ss.data();}
	float* npddetect::getScore() {return m_Scores.data();}
	...
	{
		return detect(ima.data, ima.cols, ima.rows);
	}
	...
	int npddetect::pyDetect(vector<unsigned char>& I, int width, int height)
	{
		// Clear former data.
		reset();

		// Scan with model.
		m_numScan = scan(I.data(), width, height);
