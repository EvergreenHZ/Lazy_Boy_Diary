...
namespace npd{
        ...
        int npddetect::detect(const char* imgPath)  //
        {
                ...
                vector<int> v = detect_and_align((signed char*)img.data, img.cols, img.rows);
                cv::Mat ori = cv::imread(imgPath, CV_LOAD_IMAGE_COLOR);
                if (v.size() != 0) {
                        for (int i = 0; i < 4; i++) {
                                cv::rectangle(ori, cv::Rect(v[0], v[1], v[2], v[3]), Scalar(0, 0, 255), 2);
                        }
                        for (int i = 4; i < v.size(); i += 2) {
                                circle(ori, Point(v[i], v[i + 1]), 2, Scalar(0, 0, 255), -1);
                        }
                }

                for (int i = 0; i < m_Xs.size(); i++) {
                        cout << m_Xs[i] << " " << m_Ys[i] <<" " << m_Ss[i] <<endl;
                }
                cout<<m_Xs.size()<<" faces detected"<<endl;
                //imshow("hello", img);
                //waitKey(0);
                cout << "detection finished" << endl;
                cv::imwrite("1.jpg", ori);
                return 0;
        }

        void npddetect::loadAlignModel(const char* align_model_path) {
                FILE* model = fopen(align_model_path, "rb");
        ...
        int npddetect::detect(unsigned char *I, int width, int height)
        {
                // Clear former data.
                reset();

                // Scan with model.
                m_numScan = scan((unsigned char*)I, width, height);
...
#pragma omp critical 
        ...
        vector<int> npddetect::detect_and_align(signed char I[], int width, int height)
        {
                ...
                int n = detect((unsigned char*)I, width, height);
                if (n == 0) {
                        return point_vector;
                }
                ...
                int x = m_Xs[index], y = m_Ys[index], w = m_Ss[index], h = m_Ss[index], half_size = w / 2;
